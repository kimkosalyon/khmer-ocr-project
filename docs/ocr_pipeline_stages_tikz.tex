...
\begin{tikzpicture}[
    ...
    % FIXED: Set to use your exact uploaded file name: Siemreap-Regular.ttf
    \node[input, below=0.2cm of h1] (in) {1. Input Image\\{\normalfont Grayscale Text-Line ({\fontspec{Siemreap-Regular.ttf}[Script=Khmer,Scale=0.95]អក្សរខ្មែរ})}};
    ...
    \node[seq_proc, below=1.8cm of fmap] (vpool) {4. Vertical Mean Pooling\\{\normalfont Collapses Height Dimension}};
    ...
    \node[recog, below=1.8cm of format] (bigru) {6. 2-Layer BiGRU Sequence Encoder\\{\normalfont Context Mapping (Left $\leftrightarrow$ Right)}};
    ...
    \node[shape_node, font=\small\color{teal!50!black}] (s_out) at (out.east) [xshift=0.4cm] {``{\fontspec{Siemreap-Regular.ttf}[Script=Khmer,Scale=0.95]អក្សរខ្មែរ}''};
